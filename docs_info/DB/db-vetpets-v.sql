...
-- Volcando estructura para procedimiento db-vetpets-v.add_detalle_temp
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_detalle_temp`(
...
-- Volcando estructura para procedimiento db-vetpets-v.del_detalle_temp
DELIMITER //
...
-- Volcando estructura para procedimiento db-vetpets-v.procesar_venta
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `procesar_venta`(
	IN `cod_usuario` INT,
	IN `cod_cliente` INT,
	IN `token` VARCHAR(50)
